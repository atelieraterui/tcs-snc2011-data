...
genpolnonmonic := proc (deg, var, coefrange)
    return Generate(polynom(rational(range=coefrange), var, degree=deg))
    + var ^ deg
end proc;

genpolmonic := proc (deg, var, coefrange)
    return genpolnonmonic(deg - 1, var, coefrange) + var ^ deg
end proc;

gentestpol := proc (degf, degg, degd, var, perturbf, perturbg, svdthreshold, 
                    numpol)
    # degf: degree of f
    # degg: degree of g
    ...
    for i to numpol do
        ...
        while (singularvalue < svdthreshold) do
            d[i] := evalf(genpolmonic(degd, var, -10 .. 10));
            u[i] := evalf(genpolmonic(degf - degd, var, -10 .. 10));
            v[i] := evalf(genpolmonic(degg - degd, var, -10 .. 10));
            ff[i] := expand(d[i] * u[i]);
            gg[i] := expand(d[i] * v[i]);
            fnoize0[i] := evalf(genpolnonmonic(degf - 1, var, -10 .. 10));
            gnoize0[i] := evalf(genpolnonmonic(degg - 1, var, -10 .. 10));
